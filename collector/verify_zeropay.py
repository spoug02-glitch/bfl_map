...
def main() -> None:
    ...
    # 이 숫자는 상한이다. 우리 파일은 창동씨드큐브 5km 안만 담는데 제로페이 조회는
    # 3개 구 전체라, 여기 잡힌 대부분은 애초에 반경 밖이라 안 들어온 가게다.
    # 좌표가 없으면 걸러낼 수가 없고, 좌표를 얻으려면 카카오를 불러야 한다 —
    # 그건 collect.py 의 일이다. 이 숫자를 "우리가 놓친 가게"로 읽지 말 것.
    print(f"신규 후보(우리 파일에 없음): {len(d.arrived)}곳 "
          f"— 상한이다. 5km 반경 밖이 대부분이라 실제로 들어올 수는 훨씬 적다")
    if d.unverifiable:
