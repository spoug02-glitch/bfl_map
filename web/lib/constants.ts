 ...
 * 운영 주체.
 *
 * 대표자 실명과 사업장 주소는 넣지 않는다. 전자상거래법상 사업자정보 표시의무는
 * 통신판매업자에게 걸리는데 이 서비스는 재화·용역을 팔지 않아 대상이 아니고,
 * 한 번 공개된 실명·주소는 검색엔진과 아카이브에서 되돌릴 수 없다. 상호와
 * 사업자등록번호만으로도 "사업자가 운영한다"는 사실은 확인된다.
 *
 * 메일은 임시로 개인 주소를 쓴다. lunchpick.kr 도메인이 붙으면 그때 바꾼다 —
 * 아직 없는 주소를 안내에 적으면 보낸 메일이 아무 데도 안 간다.
 */
export const CREDIT = {
  author: "노에마",
  email: "obanaeodzb@naver.com",
  bizNumber: "226-28-20577",
} as const;
