...
export type ReviewBody = { taste: number; convenience: number; body: string };
...
export function validateReviewBody(input: unknown): Result<ReviewBody> {
  ...
  if (!isRating(o.convenience)) return { ok: false, error: "점심 편의성 별점은 1~5 정수여야 합니다." };
  ...
  return { ok: true, value: { taste: o.taste, convenience: o.convenience, body: o.body } };
}
