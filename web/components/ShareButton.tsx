...
function shareUrl(r: Restaurant): string {
  ...
  return `${base}/?place=${encodeURIComponent(r.kakao_place_id)}`;
}

function shareCopy(r: Restaurant) {
  const top = r.menus[0];
  const menuPart = top?.price ? ` · ${top.name} ${Number(top.price).toLocaleString("ko-KR")}원` : "";
  return {
    title: `${r.name} · ${r.category}`.slice(0, 40),
    description: `씨드큐브에서 ${r.distance_km}km${menuPart}`.slice(0, 76),
  };
}
...
export default function ShareButton({ restaurant }: { restaurant: Restaurant }) {
  ...
  const share = async () => {
    ...
    const { title, description } = shareCopy(restaurant);
    try {
