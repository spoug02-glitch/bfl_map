...
export default function EntryNotice() {
  ...
  return (
    <div
      role="status"
      aria-live="polite"
      className={`fixed inset-x-0 top-0 z-30 flex items-center gap-2 bg-yellow-300 px-4 py-2
        text-sm text-black shadow transition-opacity duration-[400ms]
        ${phase === "fading" ? "opacity-0" : "opacity-100"}`}
      style={{ paddingTop: "max(0.5rem, env(safe-area-inset-top))" }}
    >
      <p className="flex-1">⚠️ {CONVENIENCE_NOTICE}</p>
      <button
        aria-label="알림 닫기"
